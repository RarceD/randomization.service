...
public class BlockSet
{
    ...
    private void CalculateDescriptives()
    {
        ...
        for (int i = 0; i < VariableCount; i++)
        {
            ...
            foreach (BlockOfExperimentalUnits blockOfExperimentalUnit in BlocksOfExperimentalUnits)
                VariableValues.AddRange(blockOfExperimentalUnit.ExperimentalUnits.Where(inidividual => inidividual.Values[i] != Settings.MissingValue).Select(ExperimentalUnit => ExperimentalUnit.Values[i]).ToList()); //collect all valid values
    ...
    public void CalculateRank()
    {
        ...
        for (int i = 0; i < VariableCount; i++)
        {
            ...
            if (!blockMeans.Contains(Settings.MissingValue) && !blockCVs.Contains(Settings.MissingValue))
            {
