...
public class BlockOfExperimentalUnits
{
    ...
    public void CalculateDescriptives(List<Variable> Variables, bool calculateAllDescriptives = false)
    {
        ...
        for (int i = 0; i < VariableCount; i++)
        {
            ...
            validVariableValues = ExperimentalUnits.Where(ExperimentalUnit => ExperimentalUnit.Values[i] != Settings.MissingValue).Select(o => o.Values[i]).ToList();
            ...
            if (calculateAllDescriptives)
            {
                ...
                else
                {
                    ...
                    Descriptives[i].Min = Settings.MissingValue;
                    Descriptives[i].Median = Settings.MissingValue;
                    Descriptives[i].Max = Settings.MissingValue;
                    Descriptives[i].CV = Settings.MissingValue;
                    Descriptives[i].SD = Settings.MissingValue;
                }
            }
